...
function createRepl(inspector) {
  ...
  function setBreakpoint(script, line, condition, silent) {
    ...
    if (StringPrototypeEndsWith(script, '()')) {
      const expression = script.slice(0, -2);
      const expressionValue = selectedFrame ?
        Debugger.evaluateOnCallFrame({
          callFrameId: selectedFrame.callFrameId,
          expression
        }) :
        Runtime.evaluate({ expression });
      const setBreakpointResult = PromisePrototypeThen(
        expressionValue,
        ({ result, wasThrown }) => Debugger.setBreakpointOnFunctionCall({ objectId: result.objectId, condition }));
      return PromisePrototypeThen(
        setBreakpointResult,
        (bp) => {
          ArrayPrototypePush(knownBreakpoints, bp);
        }
      )
    }
