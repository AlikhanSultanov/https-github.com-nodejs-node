...
const {
  ...
  QUIC_STREAM_HEADERS_FLAGS_WEBTRANSPORT: kHeadersFlagsWebtransport,
 ...
 *   except if the stream is a webtransport stream. For a webtransport
 *   stream setting body is illegal.
 ...
 * @property {QuicStream} [webtransportSession] The webtransport session control stream.
 * @property {boolean} [webtransport] Indicates that the headers to send signal
 ...
 * @property {boolean} [enableWebtransport] Enable webtransport
 */
 ...
 * @property {boolean} [webtransport] When true, indicates that this is a header
 *   for a webtransport connection. Note, on the response, if you deny a
 *   webtransport connection, set it to false. A webtransport header can not be
 *   terminal. So we throw an error, if webtransport and terminal are both true.
 */
 ...
 * @callback OnSessionIdCallback
 * @this {QuicStream}
 * @param {bigint|undefined} sessionid Id of the session stream or undefined 
 * if no session stream is determined. 
 * @returns {void}
 */
...
setCallbacks({
  ...
  onStreamSessionId(sessionId) {
    // Called when the stream C++ handle has determined the sessionId
    console.log('onStreamSessionId', sessionId);
    debug(`stream ${this[kOwner].id} sessionid callback`, sessionId);
...
class QuicStream {
  ...
  /** @type {OnSessionIdCallback} */
  get onsessionid() {
    assertIsQuicStream(this);
    return this.#inner.onsessionid;
  }
  ...
  set onsessionid(fn) {
    ...
    if (fn === undefined) {
      ...
      inner.state.wantsSessionId = false;
    } else {
      validateFunction(fn, 'onsessionid');
      inner.onsessionid = FunctionPrototypeBind(fn, this);
      inner.state.wantsSessionId = true;
    }
  }
  ...
  sendHeaders(headers, options = kEmptyObject) {
    ...
    validateObject(headers, 'headers');
    const { terminal = false, webtransport = false } = options;
    if (terminal && webtransport) {
      throw new ERR_INVALID_ARG_VALUE(
        'webtransport and terminal can not be set simultaenously.',
        { terminal, webtransport });
    }
    const headerString = buildNgHeaderString(
      headers, assertValidPseudoHeader, true /* strictSingleValueFields */);
    const flags = terminal ? kHeadersFlagsTerminal :
      (webtransport ? kHeadersFlagsWebtransport : kHeadersFlagsNone);
  ...
  [kMakeWebtransportStream](session) {
    if (this.pending) {
      debug('pending stream enqueuing makeWebtransportStream for session', session.id);
    } else {
      debug(`stream ${this.id} makeWebtransportStream for session`, session.id);
    }
  ...
  [kSessionId](sessionId) {
    if (this.destroyed) return;
    const inner = this.#inner;
    if (onStreamSessionIdChannel.hasSubscribers) {
      onStreamSessionIdChannel.publish({
        __proto__: null,
        stream: this,
        session: inner.session,
        sessionid,
    ...
    if (typeof inner.onsessionid === 'function') {
      console.log('kSessionId pass sess', sessionId);
      safeCallbackInvoke(inner.onsessionid, this, sessionId);
    }
  }
...
class QuicSession {
  ...
  async #createStream(direction, options = kEmptyObject) {
    ...
    validateBoolean(incremental, 'options.incremental');
    validateBoolean(webtransport, 'options.webtransport');

    const validatedBody = validateBody(body);

    if (typeof body !== 'undefined' && webtransport) {
      throw new ERR_INVALID_ARG_TYPE('options.body', 'A body can not be provided in webtransport mode');
    }

    if (webtransportSession) {
      if (webtransport && webtransportSession) {
        throw new ERR_INVALID_ARG_TYPE('options.webtransport', 'webtransport can not be set for creating a webtransport stream associated with a webtransport session');
      }

      if (headers && webtransportSession) {
        throw new ERR_INVALID_ARG_TYPE('options.headers', 'headers can not be set for creating a webtransport stream associated with a webtransport session');
      }
      if (!isQuicStream(webtransportSession)) {
         throw new ERR_INVALID_ARG_TYPE('options.webtransportSession', 'webtransportSession was to be of type QuicStream');
      }
    }
    ...
    if (headers !== undefined) {
      stream.sendHeaders(headers,
                         { terminal: validatedBody === undefined && !webtransport,
                           webtransport });
    } else if (webtransport) {
      throw new ERR_INVALID_ARG_VALUE('options.webtransport',
                                      'Specifying webtransport without a header has no effect');
    }
    if (webtransportSession) {
      if (!stream[kMakeWebtransportStream](webtransportSession)) {
        throw new ERR_QUIC_OPEN_STREAM_FAILED();
      }
    }
