...
function table(tabularData, properties, inspectOptions) {
  ...
  if (setlike) {
    // `setIter`/`mapIter` already yield arrays from `previewEntries`; a real
    // Set is materialized once so the values can be read by index below.
    const array = setIter || mapIter ? tabularData : ArrayFrom(tabularData);
    const values = [];
    for (let j = 0; j < array.length; j++)
      ArrayPrototypePush(values, _inspect(array[j]));
    return cliTable([iterKey, valuesKey], [getIndexArray(array.length), values]);
  }
