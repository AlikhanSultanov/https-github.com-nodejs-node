...
const {
  ...
  class KeyObject extends NativeKeyObject {
    ...
    toCryptoKey(algorithm, extractable, keyUsages) {
      ...
      if (type === 'private' && extractable && handle.isStoreBacked())
        throw new ERR_CRYPTO_KEY_NOT_EXPORTABLE();
      switch (type) {
        case 'secret':
...
function prepareStorePrivateKey(url, options, name, optionsName = name) {
  ...
  if (options !== undefined) {
    ...
    if (passphrase !== undefined && !isStringOrBuffer(passphrase)) {
      throw new ERR_INVALID_ARG_VALUE(
        option('passphrase', optionsName),
        passphrase);
    }
...
function prepareAsymmetricKey(key, ctx, name = 'key') {
  if (isURL(key)) {
    ...
    return prepareStorePrivateKey(key, undefined, name);
  }
  ...
  if (typeof key === 'object') {
    ...
    if (isURL(data)) {
      ...
      return prepareStorePrivateKey(data, key, `${name}.key`, name);
    }
